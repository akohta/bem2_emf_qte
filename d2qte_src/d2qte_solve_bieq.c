  ...
  for(d=0;d<=md->MN;d++){
    ...
    for(s=1;s<=md->bd.sb[d].Ne;s++){
      ...
      for(i=0;i<3;i++){
        ...
        //dudt=dEzdt_bv_node_ndmtd(d,s,i,md);
...
void create_amatrix_subdomain(int did,CMD *cm,DOMD *md)
{
  ...
  for(j=0;j<cm->N;j++){
    if(fread(tG,sizeof(double complex),cm->N,fg)!=cm->N){
      printf("d2qte_solve_bieq.c, create_amatrix_subdomain(), failed to read the tG. exit...\n");
      exit(1);
    }
    for(s=0;s<cm->N;s++){
...
void create_bmatrix_subdomain(int did,CMD *cm,double complex **u)
{
  ...
  for(j=0;j<cm->N;j++){
    ...
    if(fread(tH,sizeof(double complex),cm->N,fh)!=cm->N){
      printf("d2qte_solve_bieq.c, create_bmatrix_subdomain(), failed to read the tH. exit...\n");
      exit(1);
    }
    for(s=1;s<=cm->N/3;s++){
      for(sn=0;sn<3;sn++){
